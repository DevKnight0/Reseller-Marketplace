...
//        button_make_offer = (Button) v.findViewById(R.id.offer_button);
        ...
        mDatabaseRef1 = FirebaseDatabase.getInstance().getReference("userDetails");
...
//            String desc = bundle.getString("desc");
            ...
            seller.setText(upload.getUserName());
            sellDate.setText(upload.getDate());
            if (upload.getDesc() != null) {
                Desc_tag.setVisibility(View.VISIBLE);
                Desc_text.setVisibility(View.VISIBLE);
                Desc_text.setText(upload.getDesc());
            }
...
//                pImage.setImageBitmap(bitmapImage);
            if (upload.getImageUrl() != null) {
                String photoUrl = upload.getImageUrl();
                Glide.with(this)
                        .load(photoUrl)
                        .into(pImage);
            }
...
//        final Upload selectedItem = mUploads.get(position);
        final String selectedKey = upload.getKey();

        StorageReference imageRef = mStorage.getReferenceFromUrl(upload.getImageUrl());
        imageRef.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                int eid= upload.getEmail().indexOf('@');
                String seid= upload.getEmail().substring(0,eid);
                startActivity(new Intent(getActivity(), DrawerActivity.class));
                mDatabaseRef.child(selectedKey).removeValue();
                mDatabaseRef1.child(seid).child(selectedKey).removeValue();
